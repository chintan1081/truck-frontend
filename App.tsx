  ...
  const renderMainContent = () => {
    ...
    return (
      <Routes>
        ...
        <Route path="/fuel-management" element={<FuelManagementView fuelSites={fuelSites} fuelTransactions={fuelTransactions} trucks={fleet} drivers={drivers} orders={orders} clients={clients} sites={sites} banks={banks} onUpdateFuelTransactions={handleUpdateFuelTransactions} onAddExpense={(exp) => handleUpdateExpenses(prev => [...prev, exp])} onAddTruck={handleQuickAddTruck} onAddDriver={handleQuickAddDriver} onAddFuelSite={fs => handleUpdateFuelSites(prev => [fs, ...prev])} />} />
  ...
  return (
    <>
      <Layout
        ...
        onLogout={onLogout}
      >
        <ErrorBoundary variant="inline" resetKeys={[activeTab]}>
          {renderMainContent()}
        </ErrorBoundary>
      </Layout>
